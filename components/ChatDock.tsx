 ...
 * - Body scrolls; web component fills the panel
...
export default function ChatDock({ open, onOpenChange }: Props) {
  ...
  const elRef = useRef<OpenAIChatKitElement | null>(null);
  const [ready, setReady] = useState(false);
  const [error, setError] = useState<string | null>(null);

  // Wait for custom element definition (loaded by <Script> in layout.tsx)
  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
  ...
  useEffect(() => {
    ...
    const el = document.createElement("openai-chatkit") as unknown as OpenAIChatKitElement;
      ...
      api: {
        async getClientSecret(current?: string) {
          try {
            ...
            if (!r.ok) {
              throw new Error(`${path} ${r.status} ${typeof body === "string" ? body : JSON.stringify(body)}`);
            }

            const cs = (typeof body === "object" && body && (body as Record<string, unknown>)["client_secret"]) as
              | ClientSecret
              | undefined;
            return cs;
          } catch (e) {
    ...
    <div className={["fixed inset-0 z-[70]", open ? "pointer-events-auto" : "pointer-events-none"].join(" ")}>
      ...
      >
        ...
        <div className="flex items-center justify-between gap-3 px-4 py-3 border-b border-white/10">
          ...
          <button onClick={() => onOpenChange(false)} className="text-xs text-slate-300 hover:text-white">
            閉じる
          </button>
        </div>
